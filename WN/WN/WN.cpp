...
void loadG() {
	...
	ifstream infile;
	infile.open("C:\\Users\\Administrator\\Desktop\\pj2\\dataset\\20000-20.txt", ios::in);
	char c[200];
	int a, b;
	while (!infile.eof()) {
		infile >> a >> b;
	...
	for (int i = 0; i < pointNum; i++) {
		...
		while (q.size() != 0) {
			...
			for (it = G[curPoint].begin(); it != G[curPoint].end(); it++) {
				...
				Nk[i].push_back(tuple2(*it,curFloor));
				if (curFloor == 1) {
					validCircle[i]++;
				}
			}
...
int run() {
	...
	while (curPoint != -1) {
		...
		for (it = Nk[curPoint].begin(); it != Nk[curPoint].end(); it++) {
			if (pointValid[it->a] == false) {
				continue;
			}
			delPoint(it->a);
			vector<tuple2> ::iterator it2;
			for (it2 = Nk[it->a].begin(); it2 != Nk[it->a].end(); it2++) {
				if (pointValid[it2->a] == false) {
					continue;
				}
				if (it2->b == 1) {
					validCircle[it2->a]--;
					changePos(heapPos[it2->a]);
				}
			}
		}
